import { useState, useEffect } from 'react';
  import { DollarSign, AlertCircle, CheckCircle, Send, PlusCircle, X, Save } from 'lucide-react';
...
export default function FinancesPage() {
  ...
  const [form,    setForm]    = useState({ student_id:'', amount:'', concept:'Cuota mensual', month, year });
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={()=>{setModal(true);setForm({student_id:'',amount:'',concept:'Cuota mensual',month,year});}}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex bg-gray-900 border border-gray-800 rounded-xl p-1 w-fit">
            {['resumen','deudores'].map(t=>(
              <button key={t} onClick={()=>setTab(t)}
                className={`px-4 py-2 rounded-lg text-sm font-medium capitalize transition-colors ${tab===t?'bg-gray-800 text-white':'text-gray-400 hover:text-white'}`}>
                {t}
              </button>
            ))}
          </div>
                ...
                <>
                  ...
                  <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-gray-800">
                          {['Alumno','DNI','Cinturón','Teléfono'].map(h=>(
                            <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-800">
                        {debtors.map(d=>(
                          <tr key={d.id} className="hover:bg-gray-800/50">
                            ...
                            <td className="px-4 py-3 text-gray-400 text-sm capitalize">{d.belt}</td>
